...
if($p!=$cp)
{
	...
	{
		$w1= 'SELECT `group`,branch,email_id FROM admin_user;';
...
if($totalrows>0)
{
	do
	{
		...
		$gid1=$row_recordset['group'];
		$bid1=$row_recordset['branch'];
	    $em1=$row_recordset['email_id'];
	
		if ( $em==$em1 && $bid==$bid1 && $gid==$gid1 )
		{ 
...
$w= 'INSERT INTO admin_user( name, company_name, email_id, `group`, branch, phone,  password, confirm_password) VALUES ("'.$n.'","'.$cn.'","'.$em.'","'.$gid.'","'.$bid.'","'.$ph.'","'.$p.'","'.$cp.'")';
$x=mysql_query($w,$a1) or die(mysql_error());
	...
	if($x)
	{
		...
		header('Location: branchadmin_reporter.php');
	}
